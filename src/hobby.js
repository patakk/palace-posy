...
Ported to javascript from the PyX implementation (http
 * ://pyx.sourceforge.net/)
Copyright (C) 2011 Michael Schindler <m-schindler@us
 * ers.sourceforge.net>
...
This file re-implements some of the functiona
 * lity of MetaPost
(http://tug.org/metapost). MetaPost was developed by John D.
 *  Hobby and
others. The code of Metapost is in the public domain, which we und
 * erstand as
an implicit permission to reuse the code here (see the comment at

 * http://www.gnu.org/licenses/license-list.html)
This file is based on the Meta
 * Post version distributed by TeXLive:
svn://tug.org/texlive/trunk/Build/source
 * /texk/web2c/mplibdir revision 22737 #
(2011-05-31)
...
var mp_solve_choices = function (p, q, n, delta_x, delta_y, delta, psi) {
        ...
        if (k == 0) {
            if (s.rtype == mp_given) {
                if (t.ltype == mp_given) {
                    ...
                    // tuple([ct, st]) = mp_n_sin_cos((p.right_given() - aa)); tuple([cf, sf]) =
                    // mp_n_sin_cos((q.left_given() - aa));
                    let ct_st = mp_n_sin_cos(p.right_given() - aa);
                    ct = ct_st[0];
                    st = ct_st[1];
            ...
            if (s.ltype == mp_end_cycle || s.ltype == mp_open) {
                ...
                dd = mp_take_fraction(
                    delta[k],
                    fraction_three - mp_make_fraction(unity, Math.abs(r.right_tension()))
                );
                bb = mp_make_fraction(unity, 3 * Math.abs(t.left_tension()) - unity);
                ee = mp_take_fraction(
                    delta[k - 1],
                    fraction_three - mp_make_fraction(unity, Math.abs(t.left_tension()))
                );
                ...
                if (s.ltype == mp_end_cycle) {
                    ...
                    theta[n] = aa;
                    vv[0] = aa;
                    // k_val = range(1, n); for (k_idx in k_val) {
                    for (var k = 1; k < n; k++) {
                        // k = k_val[k_idx];
                ...
                if (s.ltype == mp_curl) {
                    ...
                    theta[n] = -(
                        mp_make_fraction(mp_take_fraction(vv[n - 1], ff), (fraction_one - mp_take_fraction(ff, uu[n - 1])))
                    );
                    break;
                } else {
                    if (s.ltype == mp_given) {
    ...
    // k_val = range((n - 1), -1, -1); for k in range(n-1, -1, -1): for (k_idx in
    // k_val) {
    ...
    while (true) {
        t = s.next;
        // tuple([ct, st]) = mp_n_sin_cos(theta[k]); tuple([cf, sf]) =
        // mp_n_sin_cos((-(psi[k + 1]) - theta[k + 1]));
        let ct_st = mp_n_sin_cos(theta[k]);
        let ct = ct_st[0];
        let st = ct_st[1];
        let cf_sf = mp_n_sin_cos((-(psi[k + 1]) - theta[k + 1]));
        let cf = cf_sf[0];
        let sf = cf_sf[1];
...
var mp_set_controls = function (p, q, delta_x, delta_y, st, ct, sf, cf) {
    ...
    if (p.right_tension() < 0 || q.left_tension() < 0) {
        if (st >= 0 && sf >= 0 || st <= 0 && sf <= 0) {
            sine = (
                mp_take_fraction(Math.abs(st), cf) + mp_take_fraction(Math.abs(sf), ct)
            );
            if (sine > 0) {
                sine *= 1.00024414062;
    ...
    p.rx_pt = (
        p.x_pt + mp_take_fraction((mp_take_fraction(delta_x, ct) - mp_take_fraction(delta_y, st)), rr)
    );
    p.ry_pt = (
        p.y_pt + mp_take_fraction((mp_take_fraction(delta_y, ct) + mp_take_fraction(delta_x, st)), rr)
    );
    q.lx_pt = (
        q.x_pt - mp_take_fraction((mp_take_fraction(delta_x, cf) + mp_take_fraction(delta_y, sf)), ss)
    );
    q.ly_pt = (
        q.y_pt - mp_take_fraction((mp_take_fraction(delta_y, cf) - mp_take_fraction(delta_x, sf)), ss)
    );
    p.rtype = mp_explicit;
    q.ltype = mp_explicit;
};
...
var map = function(v, v1, v2, v3, v4){
...
var gethobbypoints = function (knots, cycle, det = 12) {
    var hobbypts = [];
    for (var i = 0; i < knots.length - 1; i++) {
        ...
        if (algorithm == 0) {
            for (let st = 0; st < steps; st++) {
                let t = map(st, 0, steps, 0, 1);
                let tn = map(st + 1, 0, steps, 0, 1);
                let x = (1 - t) * (1 - t) * (1 - t) * p0x + 3 * (1 - t) * (1 - t) * t * p1x + 3 * (
                    1 - t
                ) * t * t * p2x + t * t * t * p3x;
                let y = (1 - t) * (1 - t) * (1 - t) * p0y + 3 * (1 - t) * (1 - t) * t * p1y + 3 * (
                    1 - t
                ) * t * t * p2y + t * t * t * p3y;

                let xn = (1 - tn) * (1 - tn) * (1 - tn) * p0x + 3 * (1 - tn) * (1 - tn) * tn * p1x + 3 * (
                    1 - tn
                ) * tn * tn * p2x + tn * tn * tn * p3x;
                let yn = (1 - tn) * (1 - tn) * (1 - tn) * p0y + 3 * (1 - tn) * (1 - tn) * tn * p1y + 3 * (
                    1 - tn
                ) * tn * tn * p2y + tn * tn * tn * p3y;

                var tonext = dist(xn, yn, x, y);
                totald += tonext;
            }
            steps = 1 + Math.round(totald / det);
        ...
        if (algorithm == 1) {
            ...
            var dt = 0.05;
            while (t < 1. - dt / 2) {
                x = (1 - t) * (1 - t) * (1 - t) * p0x + 3 * (1 - t) * (1 - t) * t * p1x + 3 * (
                    1 - t
                ) * t * t * p2x + t * t * t * p3x;
                y = (1 - t) * (1 - t) * (1 - t) * p0y + 3 * (1 - t) * (1 - t) * t * p1y + 3 * (
                    1 - t
                ) * t * t * p2y + t * t * t * p3y;
                hobbypts.push(new myVec(x, y));

                var tn = t + dt;
                xn = (1 - tn) * (1 - tn) * (1 - tn) * p0x + 3 * (1 - tn) * (1 - tn) * tn * p1x + 3 * (
                    1 - tn
                ) * tn * tn * p2x + tn * tn * tn * p3x;
                yn = (1 - tn) * (1 - tn) * (1 - tn) * p0y + 3 * (1 - tn) * (1 - tn) * tn * p1y + 3 * (
                    1 - tn
                ) * tn * tn * p2y + tn * tn * tn * p3y;
                var tonext = dist(xn, yn, x, y);
